...
import { ClaveRubrica, CriterioRubrica } from "@/types";

function idCalificacion(equipoId: string, documentoClave: ClaveRubrica, maestroId: string) {
  return `${equipoId}_${documentoClave}_${maestroId}`;
}

export async function obtenerCalificacionMaestro(equipoId: string, documentoClave: ClaveRubrica) {
...
export async function guardarCalificacion(data: {
  ...
  const puntajeMaximo = data.pesoRubrica ?? 25;
